...
fn summary(entries: &[(String, usize, usize, usize, f64)], packing_duration: f64) {
    println!("Summary:");
    ...
    let mut hits_tot = 0;
    for (i, (name, target, hits, nrots, duration)) in entries.iter().enumerate() {
        let ok = if hits == target { " " } else { "<" };
        println!("{i:>4}\t{name:>10}\t{nrots:>5}\t{target:>6}\t{hits:>6}\t{duration:8.2}\t{ok}");
        nrots_tot += nrots;
        target_tot += target;
        hits_tot += hits;
    }
    println!(
        "    \t          \t{nrots_tot:>5}\t{target_tot:>6}\t{hits_tot:>6}\t{packing_duration:8.2}"
    )
}

fn main() -> io::Result<()> {
    println!("Welcome to new pack. Let's see if I can finish this by Friday.");
    ...
    let mut entries = Vec::new(); // Log some information for the summary.
    ...
    summary(&entries, packing_duration);
