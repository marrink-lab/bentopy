...
fn validate(input: PathBuf, verbose: bool) -> Result<()> {
    ...
    let config = Config::parse_bent(&input.to_string_lossy(), &s)
        .with_context(|| format!("Could not parse {input:?} as a bentopy input file"))?;
    ...
    let config = match kind {
        Kind::Bent => {
            let config = Config::parse_bent(path, &s)
                .with_context(|| format!("Could not parse {path:?} as a bentopy input file"))?;
            eprintln!("Successfully parsed {path:?}.");
            config
        }
        Kind::Json => {
            let config: legacy::Config = serde_json::from_str(&s)
                .with_context(|| format!("Could not parse {path:?} as a legacy json input file"))?;
